...
def deepwalk(ctrl, graph):
    ...
    return DeepWalk_Original(args, embed_dim=ctrl.embed_dim, workers=ctrl.workers, graph=graph, logger=ctrl.logger).get_embeddings()
...
class DeepWalk_Original(object):
    ...
    def __init__(self, deep_walk_arguments, embed_dim, graph, workers, logger):
        ...
        logger.info("%s %s" % (resource.getrusage(resource.RUSAGE_SELF).ru_maxrss, "before training"))
        ...
        for i in range(iterations):
            ...
            logger.info("%s %s" % (resource.getrusage(resource.RUSAGE_SELF).ru_maxrss, "sampling"))
            if i == 0:
